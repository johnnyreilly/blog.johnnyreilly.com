...
async function publishToBlogger() {
    ...
    for (const docusaurusPath of blogPostsToPublish) {
        const { frontMatter, content } = await parseMarkdownFile(path.join(docusaurusBlogDirectory, docusaurusPath));
        ...
        const contentHtml = converter
            .makeHtml(content)
            .replace(/..\/static\/blog\//, 'https://raw.githubusercontent.com/johnnyreilly/blog.johnnyreilly.com/main/blog-website/static/blog/');
        ...
        // console.log(post)
        if (post && post.id) {
            const updatedPost = await updateBlog(docusaurusPath, post.id, title, tags, contentHtml)
        } else {
...
async function getPostByPath(docusaurusPath: string) {
    try {
        ...
        if (!monthPosts.data.items) {
            console.log(`Unable to look up Blogger post matching ${docusaurusPath} between ${startDate} and ${endDate}`);
            return null;
        }

        console.log(`Looking for Blogger post matching ${docusaurusPath} amongst the following posts:\n${monthPosts.data?.items?.map(post => `- ${post.url}`).join('\n')}\n`);

        let numCharacters = url.length > 6 ? 6 : 0; // start with at least 6 characters
        while (monthPosts.data.items &&
            ...
            if (matches.length === 1) {
                console.log(`Found single match starting with "${lookFor}": ${matches[0].url}\n`);
                return matches[0];
            } else {
                console.log(`Found ${matches.length} matches for blog starting with "${lookFor}"` +
...
async function postBlog(docusaurusPath: string, title: string, tags: string[], content: string) {
    try {
        const link = `http://blog.johnnyreilly.com/${docusaurusPath.substring(0, 4)}/${docusaurusPath.substring(5, 7)}/${makeBloggerPathFromDocusaurusPath(docusaurusPath)}.html`;
        console.log(`Publishing ${docusaurusPath} to ${link}`)
...
async function updateBlog(docusaurusPath: string, postId: string, title: string, tags: string[], content: string) {
    try {
        console.log(`Updating ${docusaurusPath}`)
        ...
        console.log(`Updated ${docusaurusPath} at ${post.data.url}`)
...
async function getBlogPostsToPublish() {
    ...
    const blogPostsExcludingHistoric = blogPosts.filter(post => post >= '2021-03-07');
