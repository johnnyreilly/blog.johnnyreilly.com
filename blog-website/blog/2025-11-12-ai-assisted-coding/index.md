...
AI is a fantastic tool. In a short time it has changed the way the whole industry builds software. Where we used to write every line ourselves, now code produced is generally a collaboration between engineer and AI tooling. The likes of GitHub Copilot, Claude Code, ChatGPT, Replit etc., all combine to make being a software developer in 2025 quite different from being a software developer in 2022. The rate of change has been giddying.

This post exists to dig into a little of the nuance around how software development has been changed by the innovations of AI, both the positives and the negatives. The tooling and approaches that have carried the industry forward, and the brand new pitfalls that now are revealing themselves.
...
A little information about myself: my name is John Reilly, I work as a software engineer for Investec; a multinational financial services group. The story of engineering at Investec is relevant to this piece as Investec has been an early and enthusiastic adopter of AI generally, and AI software tooling particularly. I'll seek to draw on this in this piece.
...
Historically you may have run multiple workshops, the output of which may have been a vague design and some wireframes. But with something like Replit you can leave a workshop with a functioning application. Crucially, you also have access to the source code of the AI generated application and so it is possible to take that, leave Replit and deploy it to your environment of choice.

Let's walk through some of the aspects of this.

### Prototype at speed

The time saved around the early stages of application development is considerable. The number of applications that would struggle to cross the chasm from idea to code is now vastly reduced. If you have an idea, it's now possible to realise some form of it in hours. That's incredibly useful.

A number of times in my career I've been confronted by the idea of an application that just doesn't work. A number of the underlying ideas are at conflict, in non-obvious ways. Discovering that information, at the point in time where you've gone to the trouble of engaging a team and are some way down the track of development, is a waste of time, money and good humour.

Using a tool like Replit to prototype drastically reduces the possibility that this might happen. By prompting AI to actually build a prototype of the app for you, it's possible to surface poorly thought through aspects of design before the actual build. This is really helpful and increases the chances of success.

### Build bespoke apps

Another advantage that tools like Replit offer, is the ability to build bespoke applications. Just recently at the company I work for, Investec, the organisation was looking at a tool to manage vendors for the organisation. There are a number of products in the marketplace that perform this function, but as they were examined, it was clear that each tool was opinionated. If we wanted to use one of the tools, it wouldn't quite fit our existing organisation processes and structures. We could use one of those tools, but we'd either be working around the differences, or adjusting the way we worked to be able to use the tool effectively.

The idea occurred: why not build our own? Historically, that would have taken a long time to achieve, and may not have been cost effective given the number of users. But maybe we could prompt our way to an application that aligned with the our processes? Three people got in a room for an afternoon and maybe became actually. They left the room with an AI generated application that was more aligned with Investec's preferences and processes.

The question then was, can we take our prototype application and "productionise" it? To achieve that we wanted:

- The application to live in our source control, where we run tooling like GitHub Advanced Security etc to ensure code quality etc
- We wanted to adjust the application tech stack towards one that more aligned with Investec's standard choices.
- We wanted our application to have a deployment pipeline and use CI/CD to deploy new versions.

We found that with a couple of engineers and five days work, we were able to achieve that. That's going from idea to a fully working application in just over a week. It's kind of mind-blowing when you think about it. My mind was blown.
...
While AI-generated applications offer tremendous advantages, they're not without their challenges. The very speed and ease that makes these tools so appealing can also mask significant issues that only become apparent later in the development process. Let's explore some of the key limitations and pitfalls.

### When app prompting fails

One of the most frustrating experiences with AI app generation occurs when you're working with a tool like Replit and it starts making changes to one part of your stack while rendering another part meaningless. I've encountered situations where Replit created an app with a Python backend and a TypeScript front end. At some point in the prompting journey though, it stopped updating the TypeScript front end and converted the Python into a full stack web application; effectively leaving half the application non-functional. The people prompting the app were not aware this had happened and it wasn't until we tried to migrate the application from Replit that we realised what had occurred. We've talked about AI saving time, but in this case it cost us a good amount of effort to unravel what had happened.
...
The issue of dependency management also becomes apparent quickly. Tools like Dependabot will often flag AI-generated applications for using outdated libraries with known vulnerabilities. This isn't necessarily the AI's fault - it's working with training data that includes examples using older versions of libraries. But it does mean that the "finished" application isn't actually ready for production without significant security updates.

Code quality is another concern. While AI can generate working code quickly, it doesn't always generate _good_ code. The applications work, but they may not follow best practices, may lack proper error handling, or may have performance issues that only become apparent under load.

AI application generation works exceptionally well when you "own the domain" - when the application functionality is self-contained and doesn't rely heavily on external integrations. However, if parts of your app functionality depend on external systems, APIs that don't exist in standardized forms, or data that isn't readily available, AI tools can create a false sense of completion. The app appears finished, but the hard work of integration is still ahead of you.
...
At Investec, as we've encountered this kind of scenario we are very much biased in the direction of security. So when this presents, we will take some time to identify resolve this in a secure way. This often involves more work; which is fine. The point to note here is that AI will not necessarily land you with production grade code.

### Design system challenges

For organizations with established design systems - and Investec certainly has one - AI tools present an interesting challenge. AI can generate visually appealing interfaces that work well functionally, but they often don't align with your company's design language and brand guidelines.

You might end up with a beautiful application that looks nothing like the rest of your company's digital properties. This disconnect between AI capabilities and organizational standards creates additional work to bring AI-generated UIs into compliance with house styles.
...
This presents an interesting dilemma: do you accept the AI's technology choices for the sake of speed, or do you invest time in modernizing the stack to align with your preferences and current best practices? The answer often depends on whether you're building a quick prototype or something intended for longer-term use.

## GitHub Copilot - Code completions

When GitHub Copilot first arrived, the initial code completions felt like autocomplete on steroids. It was nice to have, but not necessarily essential to the development process. The AI would suggest lines of code as you typed, often accurately predicting what you intended to write next. While impressive, it felt more like a sophisticated productivity enhancement than a fundamental game-changer.

It was useful though. It truly shone in environments with static typing. When working with strongly typed languages like TypeScript, C# or [insert name of your favourite statically typed language here], the AI has much more context to work with. It understands the shapes of your data, the contracts of your APIs, and the constraints of your type system. This additional context allows it to make far more intelligent suggestions, often completing entire functions or method implementations that not only compile but actually do what you intended.

I must say though, I wasn't really a convert particularly until agent mode arrived.

## GitHub Copilot - Agent mode

While the ask and edit features of GitHub Copilot were good improvements over basic code completion, agent mode was truly the game changer. This feature transformed the way I approach complex coding tasks, shifting from line-by-line assistance to higher-level problem solving.

Rather than implementing a feature myself from scratch, I can describe a feature or a bug fix in natural language, and the AI will generate the necessary code changes across multiple files. This holistic approach to coding feels more like collaborating with a junior developer who can take on significant chunks of work, allowing me to focus on reviewing and refining the output rather than getting bogged down in the minutiae of implementation. AI is just fantastic at boilerplate it turns out.
...
One of the most significant benefits of AI coding tools is how they can help engineers work with languages and frameworks they're not experts in. The AI becomes a bridge, allowing a Python developer to confidently work with TypeScript, or a frontend engineer to write backend services. This democratization of technical knowledge is genuinely powerful.
...
This creates a fundamental principle: if you don't understand the code, you shouldn't ship it. It's important to hold fast to this principle for the long term benefits that maintainable code gives you. It is easy to ignore the principle when AI makes it so easy to generate working solutions. But we pay if we do this in the long term.
...
The flip side of this is it can make some very silly decisions when writing tests. It may cover edge cases that are irrelevant, or miss important scenarios that a human would consider obvious. It often stubs out implementation that you actually want to test.
...
One unexpected consequence of AI-assisted coding is its impact on flow state. The traditional deep, uninterrupted focus that characterizes productive programming sessions becomes harder to achieve when you're constantly context-switching between writing code and reviewing AI suggestions.

Personally speaking, I derive great joy from getting deep into flow state as I build an application / a feature. It's a wonderfully meditative state and genuinely improves my mental health.

The collaborative nature of AI-assisted development, while powerful, can disrupt this meditative quality of coding that many engineers cherish. It's a trade-off between speed and the satisfying rhythm of sustained, focuses work.
