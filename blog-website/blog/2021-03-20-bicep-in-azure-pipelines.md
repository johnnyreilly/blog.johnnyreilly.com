...
image: blog/2021-03-20-bicep-in-azure-pipelines/rss.png
hide_table_of_contents: false
---
[Bicep](https://github.com/Azure/bicep) is a terser and more readable alternative language to ARM templates. There isn't yet a first class experience for running Bicep in Azure Pipelines, the same way there is for running ARM templates. This post demonstrates an approach that can be used until first class support lands.
...
ARM templates are quite verbose and not the easiest thing to read. This is a consequence of being effectively a language nestled inside another language. An alternative language has been created called Bicep which is far more readable. It transpiles down to ARM templates, in the same way that TypeScript transpiles down to JavaScript.

Bicep is quite new, but already it enjoys feature parity with ARM templates (as of [v0.3](https://github.com/Azure/bicep/releases/tag/v0.3.1)) and ships as part of the [Azure CLI](https://github.com/MicrosoftDocs/azure-docs-cli/blob/master/docs-ref-conceptual/release-notes-azure-cli.md#arm-1). However, as Bicep is new, it doesn't yet have a dedicated task in Azure Pipelines.  This should come, hopefully with the [v0.4 release](https://github.com/Azure/bicep/issues/1341).
...
There's two tasks above. The first is the native task for ARM deployments which takes our template and our parameters and deploys them. The second task takes the output variables from the first task and converts them into Azure Pipeline variables such that they can be referenced later in the pipeline. In this case using our `fullApplicationName` output variable.
