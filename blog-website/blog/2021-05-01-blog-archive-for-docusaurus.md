...
I decided to create my own blog archive in the land of Docusaurus.

I created a new page under the `pages` directory called `about.js`. It was essentially powered by webpack's [`require.context`](https://webpack.js.org/guides/dependency-management/#requirecontext) function. This allowed me to use webpack to obtain all of the blog modules: 

```js
require.context("../../blog", false, /.md/)
```

This code looks in the `blog` directory for files / modules ending with the suffix `".md"`.  Each one of these represents one of our blog 
posts.  It's returned in the form of `context` object, which contains all of the data about these modules.

By reducing over that data we can construct an array of objects called `allPosts` that could drive a blog archive screen:
...
![require.context](../static/blog/2021-05-01-blog-archive-for-docusaurus/require.context.png)

This gives you a flavour of the data available and shows how we pull out the bits that we need; `date`, `formattedDate`, `title` and `permalink`.
